...
export async function getHotPools() {
    ...
    const rows = pools
        ...
        .map(pool => {
            return `${constants.DEX_NAME_UNISWAP_V3},${constants.DEX_PROTOCOL_UNI_V3_STYLE},${pool.id},${pool.token0.symbol}/${pool.token1.symbol},${pool.token0.id},${pool.token0.decimals},${pool.token0.symbol},${pool.token0.name},${pool.token1.id},${pool.token1.decimals},${pool.token1.symbol},${pool.token1.name}`;
        });
